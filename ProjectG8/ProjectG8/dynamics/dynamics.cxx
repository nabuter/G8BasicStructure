...
/*      item            : dynamics.cxx
	...
	from template   : DusimeModuleTemplate.cxx
        ...
        date            : Wed May 20 18:23:15 2015
	category        : body file 
        description     : 
	changes         : Wed May 20 18:23:15 2015 first version
...
dynamics::dynamics(Entity* e, const char* part, const
  ...
  // myclock(),
 ControlSignalReadToken(getId(), NameSet(getEntity(), "ControlSignal", part)),
 DynStreamWriteToken(getId(),NameSet(getEntity(), "DynStream", part)),
  
  cb1(this, &dynamics::doCalculation),
  do_calc(getId(), "quadcopter dynamics, takes in controller input and feedback and gives state output", &cb1, ps)
{
  // do the actions you need for the simulation

  // connect the triggers for simulation
  do_calc.setTrigger(ControlSignalReadToken);
...
{
  bool res = true;
  CHECK_TOKEN(ControlSignalReadToken);
  CHECK_TOKEN(DynStreamWriteToken);
...
void dynamics::doCalculation(const TimeSpec& ts)
{
  ...
  case SimulationState::HoldCurrent: {
   ...
   StreamReader<ControlSignal> myControlSignalReader(ControlSignalReadToken, ts);
   
   D_MOD("The simulation is displaying the altered Dynamics ux-value\n");
   D_MOD(myControlSignalReader.data().uxcontrol);
   
   StreamWriter<DynStream> DynStreamWriter(DynStreamWriteToken, ts);
   DynStreamWriter.data().xpos =myControlSignalReader.data().uxcontrol*2.0;
   }
   catch (Exception& e)
   {
