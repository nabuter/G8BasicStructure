...
/*      item            : controller.cxx
	...
	from template   : DusimeModuleTemplate.cxx
        ...
        date            : Wed May 20 15:16:40 2015
	category        : body file 
        description     : 
	changes         : Wed May 20 15:16:40 2015 first version
...
PrimaryControlsReadToken(getId(), NameSet(getEntity(), "PrimaryControls", part)),
ControlSignalWriteToken(getId(),NameSet(getEntity(), "ControlSignal", part)),
FeedbackReadToken(getId(),NameSet(getEntity(), "FeedbackStream", part)),
// activity initialization
  // myclock(),
  cb1(this, &controller::doCalculation),
  do_calc(getId(), "takes in joystick output and feedback outputs to dynam", &cb1, ps)
{
  // do the actions you need for the simulation

  // connect the triggers for simulation
  do_calc.setTrigger(PrimaryControlsReadToken);
...
bool controller::isPrepared()
{
  ...
  CHECK_TOKEN(PrimaryControlsReadToken);
  CHECK_TOKEN(ControlSignalWriteToken);
  CHECK_TOKEN(FeedbackReadToken);
...
void controller::doCalculation(const TimeSpec& ts)
{
  ...
  case SimulationState::HoldCurrent: {
   ...
   StreamReader<PrimaryControls> myPrimaryControlsReader(PrimaryControlsReadToken, ts);
   
   D_MOD("The simulation is displaying the current ux-Joystick value\n");
   D_MOD(myPrimaryControlsReader.data().ux);
   
   StreamWriter<ControlSignal> ControlStreamWriter(ControlSignalWriteToken, ts);
  ControlStreamWriter.data().uxcontrol =myPrimaryControlsReader.data().ux*2.0;
   }
   catch (Exception& e)
   {
   D_MOD("Error while reading PrimaryControls-channel");
   }
   
   try
   {
     
   StreamReader<FeedbackStream> myFeedbackReader(FeedbackReadToken, ts);
   
   D_MOD("The simulation is showing the fedback-value received by the controler");
   D_MOD(myFeedbackReader.data().feedbackx);
   }
   catch (Exception& e)
   {
   D_MOD("Error while Controller Reads Feedback channel");
   }
